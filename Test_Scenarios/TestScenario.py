...
start_point_03.location.x = 246
start_point_03.location.y = 105
start_point_03.location.z = 2
start_point_03.rotation.pitch = 0
start_point_03.rotation.yaw = -90
...
class CarEnv_03_Cut_In:
    ...
    def __init__(self, spawn_env_veh = True):
        ...
        self.low  = np.array([243,  80, -5, -5,-5,243,  80, -5, -5,-5], dtype=np.float64)
        self.high = np.array([246,  100, 1, 1,5, 246,  100, 1, 1,5], dtype=np.float64)    
    ...
    def reset(self):    
        ...
        for vehicle in vehicle_list:
            if vehicle.attributes['role_name'] != "ego_vehicle" :
                vehicle.set_target_velocity(carla.Vector3D(0,-20,0))
    ...
    def step(self, action):
        ...
        if self.ego_vehicle.get_location().x < 245 and self.ego_vehicle.get_location().x > 243 and self.ego_vehicle.get_location().y < state[6]:
            print("===========================debug")
            reward += 1
        # If finish
        done = False
    ...
    def init_case(self):
        ...
        spawn_vehicles = []
        transform = Transform()
        transform.location.x = 238 
        transform.location.y = 155 + random.randint(-3,3)
    ...
    def spawn_fixed_veh(self):
        ...
        for transform in self.case_list[self.case_id - 1]:
            ...
            if blueprint.has_attribute('color'):
                color = '0,0,255'#random.choice(blueprint.get_attribute('color').recommended_values)
...
start_point_03_2.location.y = 120
...
class CarEnv_03_Follow_Ego:
    ...
    def init_case(self):
        ...
        transform.location.y = 140 +random.randint(-3,3)
        transform.location.z = 2
        transform.rotation.pitch = 0
        transform.rotation.yaw = -90
        transform.rotation.roll = 0
...
global start_point_03_3
start_point_03_3 = Transform()
start_point_03_3.location.x = -30
start_point_03_3.location.y = 194
start_point_03_3.location.z = 1
start_point_03_3.rotation.pitch = 0
start_point_03_3.rotation.yaw = 180
start_point_03_3.rotation.roll = 0

global goal_point_03_3
goal_point_03_3 = Transform()
goal_point_03_3.location.x = -77
goal_point_03_3.location.y = 144
goal_point_03_3.location.z = 0
goal_point_03_3.rotation.pitch = 0
goal_point_03_3.rotation.yaw = 0 
goal_point_03_3.rotation.roll = 0

class CarEnv_03_Cut_In_old:
    ...
    def __init__(self, spawn_env_veh = True):
        ...
        if self.ego_vehicle_bp.has_attribute('color'):
            ...
            self.ego_vehicle_bp.set_attribute('color', color)
            self.ego_vehicle_bp.set_attribute('role_name', "ego_vehicle")
        self.ego_vehicle = self.world.spawn_actor(self.ego_vehicle_bp, start_point_03_3)
        self.ego_vehicle.set_target_velocity(carla.Vector3D(-15,0,0))
    ...
    def global_routing(self):
        ...
        start = start_point_03_3
        goal = goal_point_03_3
        print("Calculating route to x={}, y={}, z={}".format(
                goal.location.x,
                goal.location.y,
                goal.location.z))
    ...
    def ego_vehicle_pass(self):
        global goal_point_03_3
        ego_location = self.ego_vehicle.get_location()
        if ego_location.distance(goal_point_03_3.location) < 30:
            return True
        else:
            return False
    ...
    def init_case(self):
        ...
        spawn_vehicles = []
        transform = Transform()
        transform.location.x = -48 
        transform.location.y = 184.8 
