...
class CARLAInput(PlacholderTfInput):
    def __init__(self, obs, name=None):

        super().__init__(tf.placeholder(tf.float32, [None] + list(obs.shape), name=name))
        self._shape = obs.shape
        self._output = (tf.cast(super().get(), tf.float32) - 0.5 * (obs.low + obs.high)) / (obs.high - obs.low)
